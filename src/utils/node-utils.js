...
function padLastWeek(tempDaysObject) {
    ...
    if(keys.length % DAYS_TO_DISPLAY !== 0) {
        var diff = DAYS_TO_DISPLAY - keys.length % DAYS_TO_DISPLAY;
...
function numOfWeeksInConfig() {
    return dates.length % DAYS_TO_DISPLAY === 0 ? dates.length / DAYS_TO_DISPLAY : Math.floor(dates.length / DAYS_TO_DISPLAY) + 1;
}
...
function createDateRanges(totalAmountOfWeeks) {
    ...
    for (var i = 0; i < totalAmountOfWeeks; i++) {
        ...
        // if the days in the last week is not exactly the weeksToDisplay value, use the remainder after dividing days by weeksToDisplay (minus 1 as index based)
            ...
            endOfWeek = amountOfDays % DAYS_TO_DISPLAY === 0 ? DAYS_TO_DISPLAY-1 : amountOfDays % DAYS_TO_DISPLAY - 1;
        }

        // construct date range
        // if only day in the date range i.e. aug 5 - aug 5, only show one date
        if (dates[i * DAYS_TO_DISPLAY].month + " " + dates[i * DAYS_TO_DISPLAY].date === dates[(i * DAYS_TO_DISPLAY) + endOfWeek].month + " " + dates[(i * DAYS_TO_DISPLAY) + endOfWeek].date) {
            var month = dates[i * DAYS_TO_DISPLAY].month;
            var date = dates[i * DAYS_TO_DISPLAY].date;
            datesArray.push(month + date);
        }
        else {
            var fromMonth = dates[i * DAYS_TO_DISPLAY].month;
            var fromDate = dates[i * DAYS_TO_DISPLAY].date;
            var toMonth = dates[(i * DAYS_TO_DISPLAY) + endOfWeek].month;
            var toDate = dates[(i * DAYS_TO_DISPLAY) + endOfWeek].date;
...
function createTableHeadData() {
    ...
    return createArrayOfConfigs(daysConfig, DAYS_TO_DISPLAY);
}
...
function createTableBodyData(dateCharges) {
    ...
    timeslots = createArrayOfConfigs(timeslots, DAYS_TO_DISPLAY);
